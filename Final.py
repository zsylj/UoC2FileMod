...
def lut(path):
    ...
    #对国家代码进行阵营划分（非轴心、盟军、中立）
    #苏、德、美、英、UNK
    #UNK意味着不可操控国家，例如希腊，法国，波兰等
    faction_list=[]
    sov_parent=["sov"]
    ger_parent=["ger","ss","cro","est","fin","hun","ita","rom","rsi","spa","svk","vf","bul"]
    us_parent=["us","bra","ff"]
    uk_parent=["uk","can","ind","nz","pol","sa","yug","nor","gr","aus"]
    other_parent=["fr","swe","nl","bel","sws"]
    for item in country_list:
        if item=="sov":
            faction_list.append("sov")
        elif item in ger_parent:
            faction_list.append("ger")
        elif item in us_parent:
            faction_list.append("us")
        elif item in uk_parent:
            faction_list.append("uk")
        else:
            faction_list.append("UNK")
    #print(faction_list)
    ...
    f.closed
    return id_list,country_list,s_type_list,faction_list,gen_type

id_list,country_list,s_type_list,faction_list,gen_type=lut("D:/Program Files/Steam/steamapps/common/Unity of Command 2/_packages/dlc2/data/campaigns/barbarossa.usc/units.yml")
